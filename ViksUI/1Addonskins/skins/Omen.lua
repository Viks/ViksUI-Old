...
if not Mod_AddonSkins or not IsAddOnLoaded("Omen") or not Viks.addonskins.Omen then return end
...
Mod_AddonSkins:RegisterSkin("Omen",function(Skin, skin, Layout, layout, config)
	...
	Omen.UpdateBarLabelSettings = function(self)
		...
		for i, v in ipairs(self.Bars) do
			v.Text1:SetFont(Viks.media.font,self.db.profile.Bar.FontSize)
			v.Text2:SetFont(Viks.media.font,self.db.profile.Bar.FontSize)
			v.Text3:SetFont(Viks.media.font,self.db.profile.Bar.FontSize)
		end
	end
	-- Skin Title Bar
	...
	Omen.UpdateTitleBar = function(self)
		...
		self.TitleText:SetFont(Viks.media.font,self.db.profile.TitleBar.FontSize)
