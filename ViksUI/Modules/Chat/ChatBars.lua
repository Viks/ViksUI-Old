...
if Viks.chat.enable ~= true or Viks.chat.chat_bar ~= true then return end

----------------------------------------------------------------------------------------
--	ChatBar(FavChatBar by Favorit)
----------------------------------------------------------------------------------------
local frame = CreateFrame("Frame", "ChatBar", UIParent)
frame:CreatePanel("Invisible", 16, Viks.chat.background and Viks.chat.height + 4 or Viks.chat.height - 1, "BOTTOMLEFT", UIParent, "BOTTOMLEFT", 2, Viks.chat.background and 24 or 20)
if Viks.chat.chat_bar_mouseover == true then
	frame:SetAlpha(0)
...
local function CreateButton(b, l, r, m)
	...
	b.t:SetTexture(Viks.media.blank_border)
...
CreateButton(b2, "/g", "/o")
b2:SetPoint("TOP", b1, "BOTTOM", 0, Viks.chat.background and -4 or -3)
b2:SetBackdropBorderColor(0, 0.54, 0, 1)
b2.t:SetVertexColor(0, 0.8, 0, 1)

local b3 = CreateFrame("Button", "$parentButton3", frame)
CreateButton(b3, "/p", "/i")
b3:SetPoint("TOP", b2, "BOTTOM", 0, Viks.chat.background and -4 or -3)
