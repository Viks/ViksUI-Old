...
local function LoadSkin()
	...
	PlayerTalentFrameTalentsClearInfoFrameCount:SetFont(Viks.media.font, 11, "OUTLINE")
	...
	hooksecurefunc("PlayerTalentFrame_UpdateSpecFrame", function(self, spec)
		...
		for i = 1, GetNumSpecializations(nil, self.isPet) do
			...
			if bu.selected then
				...
				bu.backdrop:SetBackdropBorderColor(unpack(Viks.media.bordercolor))
				bu.backdrop.overlay:SetVertexColor(0.1, 0.1, 0.1, 1)
				bu.border.backdrop:SetBackdropBorderColor(unpack(Viks.media.bordercolor))
			end
		end
	end)
		...
		for i = 1, MAX_TALENT_TIERS do
			for j = 1, NUM_TALENT_COLUMNS do
				...
				if bu.knownSelection:IsShown() then
					...
					bu.backdrop:SetBackdropBorderColor(unpack(Viks.media.bordercolor))
					bu.bg.backdrop:SetBackdropBorderColor(unpack(Viks.media.bordercolor))
