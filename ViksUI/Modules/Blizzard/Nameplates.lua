...
local function OnAura(frame, unit)
	if not frame.icons or not frame.unit or not Viks.nameplate.track_auras then return end
	local i = 1
	for index = 1, 40 do
		...
		if duration and match == true then
			...
			if i ~= 1 and i <= Viks.nameplate.width / Viks.nameplate.auras_size then icon:SetPoint("RIGHT", frame.icons[i-1], "LEFT", -2, 0) end
			i = i + 1
...
local function UpdateObjects(frame)
	...
	frame.hp:SetSize(Viks.nameplate.width * T.noscalemult, Viks.nameplate.height * T.noscalemult)
	...
	SetVirtualBorder(frame.hp, unpack(Viks.media.bordercolor))
...
local function SkinObjects(frame, nameFrame)
	...
	hp.level:SetFont(Viks.font.nameplates_font, Viks.font.nameplates_font_size * T.noscalemult, Viks.font.nameplates_font_style)
	hp.level:SetShadowOffset(Viks.font.nameplates_font_shadow and 1 or 0, Viks.font.nameplates_font_shadow and -1 or 0)
	...
	if Viks.nameplate.health_value == true then
		...
		hp.value:SetFont(Viks.font.nameplates_font, Viks.font.nameplates_font_size * T.noscalemult, Viks.font.nameplates_font_style)
		hp.value:SetShadowOffset(Viks.font.nameplates_font_shadow and 1 or 0, Viks.font.nameplates_font_shadow and -1 or 0)
	...
	hp.name:SetFont(Viks.font.nameplates_font, Viks.font.nameplates_font_size * T.noscalemult, Viks.font.nameplates_font_style)
	hp.name:SetShadowOffset(Viks.font.nameplates_font_shadow and 1 or 0, Viks.font.nameplates_font_shadow and -1 or 0)
	hp.oldname = oldname
	...
	cb:SetPoint("BOTTOMLEFT", hp, "BOTTOMLEFT", 0, -8-(Viks.nameplate.height * T.noscalemult))
	cb:SetStatusBarTexture(Viks.media.texture)
	...
	cb.time:SetFont(Viks.font.nameplates_font, Viks.font.nameplates_font_size * T.noscalemult, Viks.font.nameplates_font_style)
	cb.time:SetShadowOffset(Viks.font.nameplates_font_shadow and 1 or 0, Viks.font.nameplates_font_shadow and -1 or 0)
	...
	if Viks.nameplate.show_castbar_name == true then
		...
		cb.name:SetFont(Viks.font.nameplates_font, Viks.font.nameplates_font_size * T.noscalemult, Viks.font.nameplates_font_style)
		cb.name:SetShadowOffset(Viks.font.nameplates_font_shadow and 1 or 0, Viks.font.nameplates_font_shadow and -1 or 0)
	...
	if Viks.nameplate.class_icons == true then
		...
		cIconTex:SetSize((Viks.nameplate.height * 2) + 11, (Viks.nameplate.height * 2) + 11)
	...
	cbicon:SetSize((Viks.nameplate.height * 2) + 8, (Viks.nameplate.height * 2) + 8)
	...
	if Viks.nameplate.track_auras == true then
		if not frame.icons then
			...
			frame.icons:SetPoint("BOTTOMRIGHT", frame.hp, "TOPRIGHT", 0, Viks.font.nameplates_font_size + 7)
			frame.icons:SetWidth(20 + Viks.nameplate.width)
			frame.icons:SetHeight(Viks.nameplate.auras_size)
	...
	if not frame.raidicon then
		...
		raidicon:SetPoint("BOTTOM", hp, "TOP", 0, Viks.nameplate.track_auras == true and 38 or 16)
		raidicon:SetSize((Viks.nameplate.height * 2) + 8, (Viks.nameplate.height * 2) + 8)
...
local function CheckBlacklist(frame, ...)
	if Viks.nameplate.name_abbrev == true then return end
...
local function ShowHealth(frame, ...)
	...
	elseif (frame.isClass ~= true and frame.isFriendly ~= true) and Viks.nameplate.enhance_threat == true then
		SetVirtualBorder(frame.hp, unpack(Viks.media.bordercolor))
	end

	if GetUnitName("target") and frame:GetParent():GetAlpha() == 1 then
		frame.hp:SetSize((Viks.nameplate.width + Viks.nameplate.ad_width) * T.noscalemult, (Viks.nameplate.height + Viks.nameplate.ad_height) * T.noscalemult)
		frame.cb:SetSize((Viks.nameplate.width + Viks.nameplate.ad_width) * T.noscalemult, (Viks.nameplate.height + Viks.nameplate.ad_height) * T.noscalemult)
		frame.cb.icon:SetSize(((Viks.nameplate.height + Viks.nameplate.ad_height) * 2) + 8, ((Viks.nameplate.height + Viks.nameplate.ad_height) * 2) + 8)
	else
		frame.hp:SetSize(Viks.nameplate.width * T.noscalemult, Viks.nameplate.height * T.noscalemult)
		frame.cb:SetSize(Viks.nameplate.width * T.noscalemult, Viks.nameplate.height * T.noscalemult)
		frame.cb.icon:SetSize((Viks.nameplate.height * 2) + 8, (Viks.nameplate.height * 2) + 8)
	end
end
...
NamePlates:SetScript("OnUpdate", function(self, elapsed)
	...
	if Viks.nameplate.trackauras then
