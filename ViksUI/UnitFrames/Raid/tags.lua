...
oUF.Tags.Methods['freebgrid:pom'] = function(u) 
	...
	if source == "player" then
        if(c) and (c ~= 0) then return "|cff66FFFF"..oUF.pomCount[c].."|r" end 
    else
        if(c) and (c ~= 0) then return "|cffFFCF7F"..oUF.pomCount[c].."|r" end 
    end
end
...
oUF.Tags.Methods['freebgrid:fort'] = function(u) if not(UnitAura(u, GetSpellInfo(21562)) or UnitAura(u, GetSpellInfo(469))) then return "|cff00A1DE"..x.."|r" end end
...
oUF.Tags.Methods['freebgrid:vigil'] = function(u) if UnitAura(u, GetSpellInfo(114030)) then return "|cff8B4513"..x.."|r" end end
