...
local backdrop = {
	bgFile = "Interface\\AddOns\\ViksUI\\Root\\Media\\statusbar4", tile = true, tileSize = 0,
...
local function CreateRollButton(parent, ntex, ptex, htex, rolltype, tiptext, ...)
	...
	txt:SetFont(Viks.media.font, 10, "OUTLINE")
...
local function CreateRollFrame()
	...
	bind:SetFont(Viks.media.font, 10, "OUTLINE")
	frame.fsbind = bind

	local loot = frame:CreateFontString(nil, "ARTWORK")
	loot:SetFont(Viks.media.font, 10, "OUTLINE")
...
label:SetFont(Viks.media.font, 10, "OUTLINE")
...
local function MasterLoot_GiveLoot(frame)
	if LootFrame.selectedQuality >= MASTER_LOOT_THREHOLD then
		...
		CreatePopup.Q_GIVEMASTERLOOT.question = string.format(CONFIRM_LOOT_DISTRIBUTION, mlItemName, mlAssignNickname)
		ShowPopup("Q_GIVEMASTERLOOT")
	else
...
local dropdown = CreateFrame("Frame", "MasterLootDropDown", UIParent, "UIDropDownMenuTemplate")
