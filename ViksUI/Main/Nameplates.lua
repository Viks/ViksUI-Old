...
if not Viks.nameplate.enable ~= true then return end
...
DebuffWhiteList = {
	...
	--Wod [SpellName(118253)] = true,	-- Silencing Shot
	-- Mage
	...
	-- Warrior
	--Wod [SpellName(20511)] = true,	-- Intimidating Shout
	--Wod [SpellName(115798)] = true,	-- Weakened Blows
	[SpellName(115767)] = true,	-- Deep Wounds
	--Wod [SpellName(113746)] = true,	-- Weakened Armor
...
local noscalemult = mult * Viks.general.UiScale
...
if Viks.nameplate.trackauras == true then
...
local function CreateAuraIcon(parent)
	...
	button:SetWidth(Viks.nameplate.aurasize)
	button:SetHeight(Viks.nameplate.aurasize)
	...
	button.count:SetFont(Viks.media.font, 10, "THINOUTLINE")
...
local function UpdateAuraIcon(button, unit, index, filter)
	...
	button.cd:SetScript("OnUpdate", function(self)
		...
		button.cd.timer.text:SetFont(Viks.media.font, 8, "THINOUTLINE")
...
local function OnAura(frame, unit)
	if not frame.icons or not frame.unit or not Viks.nameplate.trackauras then return end
	local i = 1
	for index = 1, 40 do
		if i > Viks.nameplate.width / Viks.nameplate.aurasize then return end
		local match
		...
		if duration and match == true then
			...
			if i ~= 1 and i <= Viks.nameplate.width / Viks.nameplate.aurasize then icon:SetPoint("RIGHT", frame.icons[i-1], "LEFT", -2, 0) end
			i = i + 1
...
local function UpdateCastbar(frame)
	...
	frame:SetSize(Viks.nameplate.width, 20)
...
local function UpdateObjects(frame)
	...
	frame.hp:SetSize(Viks.nameplate.width * noscalemult, Viks.nameplate.height * noscalemult)
	...
	-- Set the name text
	if Viks.nameplate.showhealth == true and Viks.nameplate.trackauras ~= true then
...
local function SkinObjects(frame, nameFrame)
	...
	hp.level:SetFont(Viks.media.font, 8, "THINOUTLINE")
	...
	-- Create Health Text
	if Viks.nameplate.showhealth == true then
		hp.value = hp:CreateFontString(nil, "OVERLAY")
		hp.value:SetFont(Viks.media.font, 8, "THINOUTLINE")
	...
	hp.name:SetFont(Viks.media.font, 8, "THINOUTLINE")
	...
	cb.time:SetFont(Viks.media.font, 8, "THINOUTLINE")
	...
	-- Create Cast Name Text
		...
		cb.name:SetFont(Viks.media.font, 8, "THINOUTLINE")
	...
	if Viks.nameplate.trackauras == true then
		if not frame.icons then
			...
			frame.icons:SetHeight(Viks.nameplate.aurasize)
	...
	if not frame.raidicon then
		...
		raidicon:SetPoint("BOTTOM", hp, "TOP", 0, Viks.nameplate.trackauras == true and 38 or 16)
...
local function UpdateThreat(frame, elapsed)
	...
	if Viks.nameplate.enhancethreat ~= true then
		...
		if not frame.threat:IsShown() then
			if InCombatLockdown() and frame.isFriendly ~= true then
				...
				if Role == "Tank" then
					...
					frame.hp.bg:SetTexture(0.05, 0.05, 0.05, 1)
				else
...
local function ShowHealth(frame, ...)
	...
	elseif (frame.isClass ~= true and frame.isFriendly ~= true) and Viks.nameplate.enhancethreat == true then

	end

	if UnitName("target") and frame:GetAlpha() == 1 then
		frame.hp:SetSize((Viks.nameplate.width), 14)
		frame.cb:SetSize((Viks.nameplate.width), 12)
		frame.cb.icon:SetSize(((12) * 2) + 8, ((12) * 2) + 8)
	else
		frame.hp:SetSize(Viks.nameplate.width, 14)
		frame.cb:SetSize(Viks.nameplate.width, 12)
		frame.cb.icon:SetSize((12 * 2) + 8, (12 * 2) + 8)
